...
class PromiseStreamState : public PromiseStreamStateBase {
	static_assert(std::is_same<Datum, typename remove_cvr<Datum>::type>::value, "Datum type must not be const/volatile/reference-qualified");
	static_assert(std::is_same<Result, typename remove_cvr<Result>::type>::value, "Result type must not be const/volatile/reference-qualified");
	...
	using stream_sel =
		typename std::enable_if<
			Test<
				typename std::result_of<Consumer(Args&&...)>::type,
				Expect
			>::value,
			Promise<ResultType>
		>::type;
...
protected:
	...
	virtual completer_func reject_completer(std::exception_ptr);
	/* Promise proxy for result of streaming operation */
	Promise<Result> proxy_promise;
private:
	Promise<Result> always(StreamAction);
	using stream_consumer = std::function<Promise<StreamAction>(Datum)>;
	template <typename State>
	using stateful_stream_consumer =
		std::function<Promise<StreamAction>(State&, Datum)>;
	Promise<Result> do_stream(stream_consumer consumer);
	template <typename State, typename... Args>
	Promise<std::pair<State, Result>> do_stateful_stream(
		stateful_stream_consumer<State> consumer,
		Args&&... args);
	using DataFunc = std::function<Promise<StreamAction>(ensure_locked, Datum)>;
	/* Buffer */
	std::queue<Datum> buffer{};
	template <typename... Args>
	...
	void do_reject(ensure_locked, std::exception_ptr, bool consumer_failed);
};
